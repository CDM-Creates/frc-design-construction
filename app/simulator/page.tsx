...
const streetOnlyPattern = /^\d+[A-Za-z]?\s+.+\s+(Street|St|Road|Rd|Avenue|Ave|Drive|Dr|Circuit|Cct|Crescent|Cres|Close|Cl|Place|Pl|Parade|Pde|Way|Lane|Ln|Terrace|Tce|Highway|Hwy)$/i;
const completeAddressPattern = /^\d+[A-Za-z]?\s+.+\s+(Street|St|Road|Rd|Avenue|Ave|Drive|Dr|Circuit|Cct|Crescent|Cres|Close|Cl|Place|Pl|Parade|Pde|Way|Lane|Ln|Terrace|Tce|Highway|Hwy),?\s+.+?(?:,?\s+NSW)?\s+\d{4}$/i;

const cleanAddressPart = (value: string) => value.replace(/\s+/g, " ").replace(/\s*,\s*/g, ", ").trim();
const addressLookupKey = (value: string) => cleanAddressPart(value).toUpperCase();

function buildLookupAddress(streetAddress: string, suburb: string, postcode: string) {
  const street = cleanAddressPart(streetAddress).replace(/,\s*$/, "");
  const cleanSuburb = cleanAddressPart(suburb);
  const cleanPostcode = postcode.replace(/\D/g, "").slice(0, 4);
  if (completeAddressPattern.test(street)) return street;
  if (!streetOnlyPattern.test(street) || !cleanSuburb || cleanPostcode.length !== 4) return "";
  return `${street}, ${cleanSuburb}, NSW ${cleanPostcode}`;
}
...
export default function ProjectSimulator() {
  ...
  useEffect(() => {
    ...
    const query = buildLookupAddress(form.streetAddress, form.suburb, form.postcode);
    const lookupKey = addressLookupKey(query);
    if (!query || lastResolvedAddress.current === lookupKey) {
    ...
    const lookup = window.setTimeout(async () => {
      ...
      try {
        const response = await fetch("/api/site-analysis", {
          ...
          body: JSON.stringify({
            address: query,
            knownLandArea: form.knownLandArea,
            frontage: form.frontage,
            depth: form.depth,
          }),
          signal: controller.signal,
        });
        const result = await response.json() as SiteAnalysis & { error?: string; details?: string };
        if (!response.ok) throw new Error(result.details || result.error || "No matching NSW property was found.");
        if (requestId !== lookupSequence.current || controller.signal.aborted) return;

        const safeLocation = privateLocation(result, form.suburb, form.postcode);
        const fullPrivateAddress = result.fullAddress || result.matchedAddress;
        const resolvedStreet = result.addressDetails?.streetAddress || form.streetAddress;
        privateStreetAddress.current = fullPrivateAddress;
        lastResolvedAddress.current = addressLookupKey(fullPrivateAddress);
        ...
        setForm((current) => ({
          ...
          lotDp: result.lotDp ?? "",
          // The client-entered area remains separate from the cadastral area.
        ...
        if (requestId === lookupSequence.current && !controller.signal.aborted) setAddressLoading(false);
      }
    }, 850);
  ...
  }, [form.streetAddress, form.suburb, form.postcode, form.knownLandArea, form.frontage, form.depth]);
  ...
  const officialDifference = officialArea && form.knownLandArea
    ...
    : null;
  const matchedDifference = matchedArea && form.knownLandArea
    ? Math.round(((matchedArea - form.knownLandArea) / form.knownLandArea) * 100)
    : null;
    ...
    <main className="simulator-page">
      ...
      <header className="sim-hero">
        <div>
          ...
          <h1>Architectural concept<br /><em>built from your brief.</em></h1>
        </div>
        <p>Bring together the property, ambition, roadmap and design preferences in one premium briefing flow, then generate a structured concept study for client and architect review.</p>
      </header>

      <div className="access-strip">
        <span><i />{entitlement.label}</span>
        <p>Your information is carried forward automatically and is never sent to an AI provider until you confirm the final brief.</p>
        <button type="button" title={futureSubscriptionFeatures.join(", ")}>Cloud AI orchestration <span>＋</span></button>
      </div>
      ...
      <section className="sim-workspace">
        <form className="sim-form" onSubmit={submit}>
          ...
          <fieldset>
            ...
            <label className="span-2 address-entry"><span>Full NSW property address <small>Private lookup only</small></span><input required autoComplete="street-address" value={form.streetAddress} onChange={(event) => update("streetAddress", event.target.value)} placeholder="Street address, or enter the complete NSW address" /><small className="address-entry-help">Enter the street address here and the suburb/postcode below, or paste the complete NSW address. The official parcel is matched only after all parts are complete.</small></label>
            {addressLoading && <div className="address-autofill loading span-2"><i className="analysis-spinner" /><div><strong>Finding the official NSW parcel…</strong><span>Address, council, cadastral lot, mapped area and approximate dimensions are being checked.</span></div></div>}
            {!addressLoading && matchedProperty && <div className={`address-autofill matched span-2 ${matchedProperty.areaStatus === "requires_verification" ? "review" : ""}`}><i>✓</i><div><strong>Property successfully matched</strong><span>{matchedProperty.council} Council · {matchedProperty.controls.zone} {matchedProperty.controls.zoneName} · {form.lotDp || "Lot / DP unavailable"}</span><small>Mapped cadastral parcel: {matchedArea?.toLocaleString() ?? "area unavailable"} m² · client-entered plot area: {form.knownLandArea.toLocaleString()} m²{matchedDifference !== null ? ` · ${matchedDifference === 0 ? "areas agree" : `${matchedDifference > 0 ? "+" : ""}${matchedDifference}% difference`}` : ""}.</small><small>{matchedProperty.parcelArea?.status === "requires_verification" ? `${matchedProperty.parcelArea.note} Service: ${matchedProperty.serviceReportedAreaSqm ?? "n/a"} m² · geometry: ${matchedProperty.calculatedGeometryAreaSqm ?? "n/a"} m².` : "Mapped dimensions are indicative and remain subject to title, deposited plan and registered survey."}</small></div></div>}
            {!addressLoading && addressError && <div className="address-autofill failed span-2"><i>!</i><div><strong>We need a complete NSW address</strong><span>{addressError}</span></div></div>}
            <label><span>Suburb <small>{matchedProperty ? "Auto-filled" : form.suburb ? "Entered · not verified" : "Pending"}</small></span><input required value={form.suburb} onChange={(event) => update("suburb", event.target.value)} placeholder="NSW suburb" /></label>
            <label><span>Postcode <small>{matchedProperty ? "Auto-filled" : form.postcode ? "Entered · not verified" : "Pending"}</small></span><input required inputMode="numeric" pattern="[0-9]{4}" maxLength={4} value={form.postcode} onChange={(event) => update("postcode", event.target.value.replace(/\D/g, ""))} placeholder="Postcode" /></label>
            <label className="span-2"><span>Lot / DP <small>{matchedProperty && form.lotDp ? "Auto-filled from cadastral lot" : form.lotDp ? "Entered · not verified" : "Pending match"}</small></span><input value={form.lotDp} onChange={(event) => update("lotDp", event.target.value)} placeholder="Filled from the selected NSW cadastral parcel" /></label>
          </fieldset>
          ...
          <button className="run-simulation" disabled={loading || addressLoading || form.postcode.length !== 4} type="submit">{loading ? "Checking NSW planning layers…" : "Review my complete project brief"}<span>{loading ? "···" : "→"}</span></button>
