...
export function PlanningReportClient({ reportId }: { reportId: string }) {
  ...
  const secureDownload = async (kind: "pdf" | "pack") => {
    setDownloading(kind);
    setActionStatus("");
    try {
      const response = await fetch(`/api/planning-simulation/reports/${reportId}/${kind}`, { headers: { "X-FRC-Order-Token": access } });
      if (!response.ok) {
