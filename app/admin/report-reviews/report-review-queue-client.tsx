...
export function ReportReviewQueueClient() {
  ...
  const [correctionsMade, setCorrectionsMade] = useState("");
  const [unresolvedMatters, setUnresolvedMatters] = useState("");
  ...
  return (
    <main className="planning-admin-page review-queue-page">
      ...
      <section className="reviewer-identity-fields"><label><span>Reviewer name</span><input value={reviewerName} onChange={(event) => setReviewerName(event.target.value)} /></label><label><span>Reviewer role</span><input value={reviewerRole} onChange={(event) => setReviewerRole(event.target.value)} placeholder="Use a legally accurate role" /></label><label className="wide"><span>Professional observations / required changes</span><textarea value={notes} onChange={(event) => setNotes(event.target.value)} /></label><label className="wide"><span>Corrections made in this revision</span><textarea value={correctionsMade} onChange={(event) => setCorrectionsMade(event.target.value)} /></label><label className="wide"><span>Unresolved matters at release</span><textarea value={unresolvedMatters} onChange={(event) => setUnresolvedMatters(event.target.value)} /></label></section>
