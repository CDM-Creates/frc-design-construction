...
export function getProductionReadiness(): {
  ...
  const durableStorageConfigured = backend === "cloudflare" || (
    backend === "supabase" && Boolean(process.env.SUPABASE_URL && (process.env.SUPABASE_SECRET_KEY || process.env.SUPABASE_SERVICE_ROLE_KEY))
  );
  const checks: ReadinessCheck[] = [
    ...
    { code: "private_storage", label: "Private document storage", status: durableStorageConfigured ? "configured_unverified" : "mock_only", detail: backend === "cloudflare" ? "Private R2 is selected; verify retention and malware-scanning controls." : backend === "supabase" ? (durableStorageConfigured ? "Private Supabase Storage is selected; verify retention and malware-scanning controls." : "Supabase is selected but its URL or server-only secret key is missing.") : "Node private-storage fallback is selected. Durable production uploads still require Supabase Storage, R2 or another durable store." },
